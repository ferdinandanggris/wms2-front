...
const InvoicedList = ({ user, data, refreshData, deleteData, exportData, setOrderStatus }) => {
  const title = "Invoiced Order";
  const img = <FaFileInvoice className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Documents - Invoiced";
  ...
  const condition = { status: "CLOSED", paymentDate: 0 };
  const bulkAction = [{ title: "Payment", type: "payment" }];
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData, setOrderStatus })(InvoicedList);
