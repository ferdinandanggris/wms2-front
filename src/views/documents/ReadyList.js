...
const ReadyList = ({ user, data, refreshData, deleteData, exportData, setOrderStatus }) => {
  const title = "Ready Order";
  const img = <FaFileDownload className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Documents - Ready";
  ...
  const condition = { status: "CLOSED", invoiceDate: 0 };
  const bulkAction = [{ title: "Invoice", type: "invoice" }];
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData, setOrderStatus })(ReadyList);
