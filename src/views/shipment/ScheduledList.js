...
const ScheduledList = ({ user, data, refreshData, deleteData, exportData, setShipmentStatus }) => {
  const title = "Scheduled Shipment";
  const img = <FaCalendarCheck className="module-img" />;
  const path = "/shipment";
  const url = "shipment";
  ...
  const exportFilename = "scheduled.csv";
  const condition = { status: "SCHEDULED" };
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData, setShipmentStatus })(ScheduledList);
