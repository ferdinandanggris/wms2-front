...
const TransferList = ({ user, data, refreshData, deleteData, exportData }) => {
  const title = "Transfer Order";
  const img = <FaRoute className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Order - Transfer";
  ...
  const exportFilename = "transfer.csv";
  const condition = { status: "TRANSFER" };
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData })(TransferList);
