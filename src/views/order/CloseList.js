...
const CloseList = ({ user, data, refreshData, deleteData, exportData }) => {
  const title = "Closed Order";
  const img = <FaPeopleCarry className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Order - Closed";
  ...
  const exportFilename = "closed.csv";
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData })(CloseList);
