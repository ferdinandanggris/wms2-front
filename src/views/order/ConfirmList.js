...
const ConfirmList = ({ user, data, refreshData, deleteData, exportData }) => {
  const title = "Confirmed Order";
  const img = <FaVoteYea className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Order - Confirmed";
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData })(ConfirmList);
