...
const InHubList = ({ user, data, refreshData, deleteData, exportData }) => {
  const title = "In Hub Order";
  const img = <FaBoxes className="module-img" />;
  const path = "/order";
  const url = "order";
  const role = "Order - In Hub";
  ...
  const exportFilename = "inhub.csv";
  const condition = { status: "INHUB" };
...
export default connect(mapStateToProps, { refreshData, deleteData, exportData })(InHubList);
